...
|--------------|-----------------------|------------------|----------|------------------------|--------------------|--------------|
| Jetson Orin 7B local VLM verifier | Jetson Orin 64GB | `scripts/setup_jetson_venv.sh` with `configs/jetson-qwen25vl7b.yaml` | YOLOE-S FP16 (`yoloe-11s-seg.pt`) | `transformers`, `Qwen/Qwen2.5-VL-7B-Instruct` | BF16 | First-pass 7B VLM smoke candidate. Use the conservative 1-keyframe / 336 px profile and record verifier latency before promotion. See `docs/benchmarks/jetson-orin-qwen25vl7b-vlm-plan-2026-05-14.md`. |
