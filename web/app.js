 ...
 * Sample data below matches vrs/schemas.py::VerifiedAlert.to_json() and the
 * classes in configs/policies/safety.yaml. Served over HTTP it will try to load
 * ./sample_alerts.jsonl; otherwise it falls back to the embedded SAMPLE_ALERTS.
 */
"use strict";
...
function renderRuntimePanel() {
  const rows = [
    ["Detector", "YOLOE-11L · FP16", "bolt"],
    ["Verifier", "Cosmos-Reason2-2B · baseline", "cpu"],
    ["Backend", "Ultralytics → VLMBackend", "cascade"],
    ["Failure policy", "pass_through", "shield"],
  ];
  $("runtime-panel").innerHTML =
    ...
    `<div class="text-[11px] text-gray-400 dark:text-gray-500 pt-1 leading-snug">Verifier is pluggable; Qwen3.5/3.6-class comparison is the priority path.</div>`;
}
...
function renderAlerts() {
  ...
  const rows = list.map((a, i) => `
    <tr class="border-b border-gray-100 dark:border-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-700/40 cursor-pointer" data-alert="${all.indexOf(a)}">
      <td class="py-2 pl-4 pr-2">${thumbSVG(a, 132, 74)}</td>
...
function renderStreams() {
  const cards = STREAMS.map((s) => {
    ...
    return `<div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
      <div class="relative">
        ${last ? thumbSVG(last, 640, 240).replace('width="640" height="240"', 'class="w-full h-auto block" preserveAspectRatio="xMidYMid slice"') : `<div class="w-full aspect-video bg-gray-900 flex items-center justify-center text-gray-600">${icon("video", 32)}</div>`}
  ...
  $("view").innerHTML = `<div class="p-6">
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Per-camera latest keyframe. Each tile shares the single YOLOE detector and VLM verifier.</p>
...
function openDrawer(a) {
  ...
  $("drawer").innerHTML = `
    <div class="p-5">
      ...
      <div class="rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 mb-4">${thumbSVG(a, 440, 248).replace('width="440" height="248"', 'class="w-full h-auto block"')}</div>
      <div class="grid grid-cols-2 gap-3 mb-4 text-sm">
        ...
        <div class="rounded bg-gray-50 dark:bg-gray-900 px-3 py-2"><div class="text-xs text-gray-400">Detector score</div><div class="font-semibold">${d.score.toFixed(2)}</div></div>
...
/* Try a real run's alerts.jsonl when served over HTTP; keep samples otherwise. */
async function tryLoadReal() {
  try {
    const res = await fetch("./sample_alerts.jsonl", { cache: "no-store" });
    if (!res.ok) return;
    const text = await res.text();
    const recs = text.trim().split("\n").filter(Boolean).map((l) => JSON.parse(l));
    if (recs.length) {
      state.alerts = recs.map((r) => ({ ts: r.ts || new Date().toISOString(), stream_id: r.stream_id || "cam-01", ...r }));
      render();
    }
  } catch (_) {
    /* file:// or no file — embedded SAMPLE_ALERTS already in place */
  }
}
...
function init() {
  ...
  setInterval(maybePushAlert, 3000);
  tryLoadReal();
}
