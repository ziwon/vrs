...
VRS is a two-stage CCTV / video-understanding cascade sized for a single 16 GB GPU:
...
- **Slow path** — NVIDIA Cosmos-Reason2-2B (Qwen3-VL-2B post-train) verifies each candidate alert with CoT + normalized bbox + trajectory output. It is invoked only after `EventStateQueue` promotes a detection run.

## Common commands

```bash
# Install (pick the torch wheel for your GPU arch; see README for cu128/cu121 notes)
conda create -n vrs python=3.12 -y && conda activate vrs
...
python scripts/run_mp4.py     --video <path>  --config configs/default.yaml --policy configs/policies/safety.yaml --out runs/demo
python scripts/run_rtsp.py    --rtsp  <url>   --config configs/default.yaml --policy configs/policies/safety.yaml --out runs/live

# Run multi-stream (shared YOLOE + shared Cosmos, N cameras)
...
python scripts/eval.py --dataset <dir> --config configs/default.yaml \
...
python -m vrs.eval.ci --baseline runs/baseline/report.json \
  --current runs/eval/report.json --max-f1-drop 0.02

# Tests (CPU-only smoke tests — no CUDA / YOLOE / Cosmos required)
pytest -q
pytest -q tests/test_smoke.py::test_event_state_requires_min_persist    # single test
```
...
5. `vrs/runtime/cosmos_loader.py` — loads Cosmos-Reason2-2B in `bf16`/`fp16`/`w4a16`. `chat_video(..., response_schema=...)` handles BGR→PIL RGB, the Qwen3-VL `{type:"video", video:[...], fps:N}` message schema, and processor-version quirks (falls back to `images=` when the processor doesn't accept `videos=`). When `response_schema` is set it calls `_build_logits_processor` (which delegates to `vrs/verifier/constrained.build_logits_processor`) and passes the resulting XGrammar `LogitsProcessor` to `generate`. The class conforms to the `CosmosBackend` Protocol in `vrs/runtime/backends.py`; alternate implementations (`VLLMCosmosBackend` in `vllm_cosmos.py`, reserved `trtllm`) plug in through `build_cosmos_backend(cfg, backend=...)` and handle their own guided-decoding surface internally — `AlertVerifier` only ever touches `chat_video`.
6. `vrs/verifier/alert_verifier.py` — builds the strict-JSON prompt, calls Cosmos, parses the response with a balanced-brace JSON extractor (`_find_json_object`) that tolerates LLM prose and nested braces. Verifier failures are governed by `verifier.failure_policy`: default `pass_through` returns `true_alert=True, confidence=0.0` with a diagnostic rationale so detector hits are not silently dropped; `reject` returns `true_alert=False, confidence=0.0` for deployments that prioritize false-alarm suppression. When the optional `xgrammar` dep is installed (`pip install 'vrs[constrained]'`), `vrs/verifier/constrained.py` compiles the response schema (with `false_negative_class` restricted to policy class names + null) into a transformers `LogitsProcessor`, and a fresh processor is passed to `generate()` per `verify()` call — parse failures become structurally impossible and the balanced-brace parser becomes a safety net only.
...
RTSP[i] ─► DecoderThread[i] ──► frame_q (BoundedQueue, drop_oldest)
                         ...
                         DetectorWorker (batched YOLOE; owns all EventStateQueues)
                                  │
                                  ▼ candidate_q
                         VerifierWorker (Cosmos-Reason2-2B, shared)
                                  │
                                  ▼ per-stream sink_q[i]
...
Key ownership rules:
- Only the `DetectorWorker` thread touches the YOLOE CUDA model; only the `VerifierWorker` touches Cosmos. This avoids CUDA context races without explicit locking.
...
- `configs/default.yaml` — single GPU (~8.5 GB): YOLOE-L FP16 + Cosmos-Reason2-2B BF16.
- `configs/tiny.yaml` — 8 GB / Jetson (~3.5 GB): YOLOE-S FP16 + `embedl/Cosmos-Reason2-2B-W4A16`.
...
- **Dates/versions:** project is on Python 3.12+, `transformers>=4.46`, `ultralytics>=8.3.0`. Default detector is `yoloe-11l-seg.pt`; tiny profile uses `yoloe-11s-seg.pt`.
